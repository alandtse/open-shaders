...
-   **DLL Deployment Invariant:** After any C++ change, explicitly rebuild the `CommunityShaders` target before running the AIO packaging target. Confirm the Release DLL timestamp/hash changed as expected, and verify the deployed `CommunityShaders.dll` SHA-256 matches the freshly built Release DLL. The AIO target may package a stale DLL without recompiling it.
