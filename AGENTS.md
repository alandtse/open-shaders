...
-   **Feature Boundaries:** A non-core feature must not depend on another non-core feature (calling its methods, reading its settings, branching on its `IsEnabled()`/`loaded`) — guarded or not, either side can be absent from a given build, so this chains one missing feature into several broken ones. Depending on an `IsCore()` feature is fine (always compiled and bundled). Don't add feature-specific code, state, or hooks to `Hooks.cpp`/`State.cpp`/`Globals.h`; use the standard per-feature hook points instead.
...
-   **Feature Isolation:** Avoid dependency chains between features that aren't guaranteed to ship together. A _non-core_ feature must not depend on another non-core feature — calling its methods, reading its settings, or branching on its `IsEnabled()`/`loaded` state, guarded or not — because either one can be absent from a given build (see AIO Bundling Semantics above), turning one missing feature into several broken ones. Depending on an `IsCore()` feature is fine: core features are always compiled and bundled, so that dependency can't go missing. Central engine files (`Hooks.cpp`, `State.cpp`/`.h`, `Globals.h`) must stay feature-agnostic — don't add a branch, field, or hook to them for one feature's benefit. If a technique needs a new engine interception point, expose it as a hook any feature could use (a new per-feature virtual/override on `Feature`, see `docs/new-feature-template/`), not code wired to one feature by name.
