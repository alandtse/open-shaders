...
-   **Linux/macOS-host cross-compile (build-only, no VS/Windows tooling):** `cmake --preset Linux-ClangCL && cmake --build --preset Linux-ClangCL`. Cross-compiles to the same Windows PE/MSVC-ABI output as the native presets via `clang-cl`+`lld-link` against an `xwin`-generated Windows SDK/CRT sysroot (the toolchain and DirectXTK's Wine-based shader-compile workaround both come from the `extern/CommonLibSSE-NG` submodule; see `extern/CommonLibSSE-NG/examples/linux-cross-compile/README.md` for one-time host setup — xwin, wine, llvm-mingw, vcpkg). Proves the toolchain compiles clean; does not produce a runnable-in-game package, and struct-layout/vtable mismatches between clang-cl and real MSVC only surface as an in-game crash, not a build failure.
    -   **Known gap:** CMake configure succeeds fully, and all vcpkg dependencies build clean (three new overlay ports needed beyond CommonLibSSE-NG's own example: `cppwinrt` — the official port hard-requires real Windows SDK `.winmd` metadata that `xwin` doesn't stage, so this one runs `cppwinrt.exe` under Wine with `-base` instead, generating only the base COM-interop headers this codebase actually uses; `detours` — the official port's NMake-based build needs a real `nmake.exe`, so this one compiles the same source-file set via a small vendored `CMakeLists.txt` instead; `directxtex` — same Wine/`fxc2` shader-compile workaround as DirectXTK, adapted). The one unresolved piece is `extern/FidelityFX-SDK`'s bundled shader compiler, `FidelityFX_SC.exe`: it has genuine, deterministic parsing bugs when run under Wine (confirmed reproducible with a fresh Wine prefix and `WINEESYNC=0`/`WINEFSYNC=0`, so not flaky) — e.g. it treats `pass` as a reserved keyword, colliding with its own shipped use as a plain parameter name in `ffx_cacao_callbacks_hlsl.h`, and rejects some `switch`/`case`/`globallycoherent` patterns. This affects FSR1/FSR2/FSR3 Upscaler shaders (which this codebase does use), not just frame interpolation/optical flow (which the preset disables via `-DFFX_FI=OFF -DFFX_OF=OFF` as an unrelated, worthwhile scope reduction). Whether the vendored `FidelityFX_SC.exe` also fails this way on native Windows is unverified. Until resolved, the `Linux-ClangCL` build fails at FidelityFX-SDK's shader-generation step; everything before that (CMake configure, all vcpkg deps, C++ compilation of our own code and CommonLibSSE-NG's prebuilt linkage) is verified working.
