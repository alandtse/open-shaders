...
#	endif
	...
	if (treeBendEnabled) {
		...
		float3 previousTreeRootWS =
			float3(PreviousWorld[eyeIndex][0].w, PreviousWorld[eyeIndex][1].w, PreviousWorld[eyeIndex][2].w);
#	if defined(SKINNED)
		float3 absoluteTreeRootWS = treeRootWS;
		float3 previousAbsoluteTreeRootWS = previousTreeRootWS;
#	else
...
#	endif
		...
		currentTreeImpact = Wind::Tree::ResolveTransientImpact(
			...
			SharedData::SampleCurrentTransientWindImpulses(currentTreeMiddleWS),
			SharedData::SampleCurrentTransientWindImpulses(currentTreeTopWS));
