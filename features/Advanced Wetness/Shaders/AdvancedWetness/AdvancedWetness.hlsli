...
namespace WetnessEffects
{
	...
	float4 GetRainDrops(float3 worldPos, float t, float3 normal, float rippleStrengthModifier = 1.0, float2 flowOffset = float2(0.0, 0.0))
	{
		...
		bool useLegacyRainBehavior = SharedData::advancedWetnessSettings.EnableLegacyRainBehavior != 0;
		...
		[branch] if (useLegacyRainBehavior && enableRipples) {
			const float gridSize = rcp(max(SharedData::advancedWetnessSettings.RaindropGridSizeRcp, 1e-3));
			const float chaoticScaleRcp = rcp(max(gridSize * 5.0, 1e-3));
			const float chaoticSpeed = max(0.2, SharedData::advancedWetnessSettings.RaindropIntervalRcp * 0.5);
			const float chaoticStrength = SharedData::advancedWetnessSettings.RippleStrength * rippleStrengthModifier * 0.25;
