...
-   `velocity`: the resulting 3D ambient velocity.
...
front with a turbulent detail layer, then applies the configured contrast and
gust range.

The along-wind coordinate is advected by the accumulated travel distance:

```text
advection = gustTravelDistance * advectionUnitsPerSecond
alongCoordinate = (alongWind - advection) / gustScale
```
...
`SampleWind` remains pure. The frame-level owner is `State`, which integrates a
single non-negative travel distance once per frame:

```cpp
gustTravelDistance += max(selectedWindSpeed, 0.0f) * deltaTime;
```

This treats speed as instantaneous velocity. A speed decrease can reduce the
next frame's advance, but cannot move the field backward because the
accumulated distance never decreases.
...
2. Higher override speed must advance gust fronts faster.
