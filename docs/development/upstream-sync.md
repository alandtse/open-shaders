...
git merge --no-ff --no-edit \
    ...
    upstream/dev
# On conflicts (always in non-fork-owned paths -- see .gitattributes): resolve
# each hunk by hand, never with -X theirs/-X ours or checkout --theirs/--ours.
...
1. **Resolve conflicts favoring the fork's side by default** (this fork is the VR maintainer) — see [Commit structure](#commit-structure-for-a-conflicted-sync) for the per-hunk process this now runs through. Typical conflicts are just fork CI config or feature `.ini` versions, where keeping ours is almost always right; still check per rule 8 whether upstream's side also fixes something real.
...
13. **Verify every fork-added line survived, mechanically, not just by reviewer impression.** Diff the fork's pre-sync tree against its own merge-base ancestor to get every fork-added line (`+` lines only), then check each one still exists somewhere in the post-sync tree; a missing line needs either a restoration or a stated reason it was deliberately dropped, recorded in the merge or tail commit body. A symbol still existing elsewhere in the file is not evidence that a specific line survived -- check the line itself, not just a name it happens to share with unrelated code nearby. This doesn't replace rules 6-12; it catches, before a person has to reconstruct it by hand, the exact class of loss those rules describe.

### Commit structure for a conflicted sync

**Never resolve a sync's conflicts with `-X theirs`, `-X ours`, or `git checkout --theirs/--ours <file>`.** Those apply one answer to an entire file at once, including hunks nobody actually looked at — and on this fork, that has cost real fork content: a genuine, git-flagged conflict resolved that way leaves no marker and no `--diff-filter=U` entry, so a later review pass has to rediscover the loss from scratch instead of ever seeing it as a conflict in the first place. (An automated job that hits any conflict at all already aborts and hands off to manual resolution — see [When the workflow halts](#when-the-workflow-halts) — so this only affects the manual path, not the weekly unattended run.)

Merge with no strategy option instead, and resolve what's left by hand, one hunk at a time:

1. Before resolving anything, capture the unresolved state (`git diff > conflicts.patch`, or note the conflicted file list) so the PR carries a record of what git actually flagged, not just the final result.
