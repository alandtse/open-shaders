...
-   **Present-tense Comments**: Code comments describe what the code _is_, not what it used to be. Phrases like "previously", "we used to", "this replaces", "originally", "the old version", or references to specific past incidents/PRs/SHAs do not belong in code. The reader is looking at the present file and doesn't have the history loaded. Exception: when there's a real regression risk a future maintainer needs to avoid — a "do not revert this guard, the absence caused X bug" note is load-bearing and stays. Frame the risk in present tense and only as much history as the reader needs to avoid the trap. Commit messages, PR descriptions, and CHANGELOG entries are the right place for "what changed" — code comments are not.
