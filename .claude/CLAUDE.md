...
-   **DRY review is codebase-wide, not diff-local**: A DRY check on a PR/diff is not just "does this PR repeat itself" — it is "does this change reinvent functionality that an existing shared utility already provides, and should call instead." Review the diff **against the rest of the tree**: every new `device->Create*` must use `Util::SetResourceName`; a game-setting read should use `Util::GetGameSettingValue`; a VR check should use `globals::game::isVR`; module/image bounds, JSON (de)serialization, format/path helpers, etc. should reuse the canonical `Utils/*` helper rather than a hand-rolled equivalent. Flag a reimplementation even when the duplication is only against existing code and not visible within the PR itself, and prefer switching the new code to the shared utility.
