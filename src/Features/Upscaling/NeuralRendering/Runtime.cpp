...
namespace NR
{
	namespace
	{
		...
		void WriteCreation(NVSDK_NGX_Parameter* a_parameters, const CreationWrites& a_writes)
		{
			...
			SetInt(a_parameters, "Feature_Flags", kFeatureFlags);
			SetInt(a_parameters, "NVSDK_NGX_Parameter_Feature_Flags", kFeatureFlags);
			...
			SetInt(a_parameters, "DLSSNR.Hdr", 1u);
			SetInt(a_parameters, "DLSSNR.SDR", 0u);
	...
	AttemptResult Runtime::Initialize(D3D12Interop& a_interop, const std::filesystem::path& a_directory)
	{
		...
		try {
			...
			logger::info("[NeuralRendering] Feature 18 runtime initialized ({}) on adapter LUID {:08X}:{:08X}",
		...
		FrameParameters& a_frame, const Tuning& a_tuning)
	{
		...
		if (!eye.feature) {
			...
			if (NVSDK_NGX_FAILED(created) || !handle) {
				logger::error("[NeuralRendering] Eye {} Feature 18 creation failed: 0x{:08X} (flags=0x{:X})", a_eyeIndex, a_frame.result, kFeatureFlags);
				return false;
			}
			logger::info("[NeuralRendering] Eye {} Feature 18 instance created (flags=0x{:X})", a_eyeIndex, kFeatureFlags);
		...
		if (NVSDK_NGX_FAILED(evaluated)) {
			logger::error("[NeuralRendering] Eye {} evaluation failed: 0x{:08X}", a_eyeIndex, a_frame.result);
			return false;
		}
		return true;
	}
