...
namespace NR
{
	 ...
	 *        successful drain of the same layer may release them. A removed device is excluded:
	 *        it never retires anything again, so draining it first would retain for good.
	 */
	[[nodiscard]] constexpr bool IsRetainedFailure(FailureKind a_kind)
	{
		return a_kind == FailureKind::kDrainTimeout || a_kind == FailureKind::kWaitFailed;
	}
	...
	/** @brief Which plain-language line the settings menu shows for the pass. */
	enum class DisplayState : uint8_t
	{
		kOff,              ///< The feature toggle is off.
		kStarting,         ///< Enabled, but the runtime layer has not finished initializing.
		kActive,           ///< The runtime is live and frames are compositing.
		kNoGpu,            ///< The renderer's adapter is not an NVIDIA one.
		kDllMissing,       ///< The runtime DLL is not installed.
		kVersionRejected,  ///< The DLL is present, but not a supported 310.8.x build.
		kFailed            ///< Latched on anything else, including a pass-level failure.
	};

	/** @brief Diagnostics label for a displayed state. */
	[[nodiscard]] constexpr std::string_view DisplayStateName(DisplayState a_state)
	{
		switch (a_state) {
	...
	/**
	 * @brief The line the menu shows, ordered by what a user can act on: the toggle first, then the
	 *        adapter, then a pass-level latch, then the runtime layer's own latch.
	 */
	[[nodiscard]] constexpr DisplayState ClassifyDisplayState(bool a_enabled, bool a_adapterSupported,
		RuntimeState a_layer, FailureKind a_failure, bool a_passFailed)
	{
		if (!a_enabled)
			return DisplayState::kOff;
		if (!a_adapterSupported)
			return DisplayState::kNoGpu;
		if (a_passFailed)
			return DisplayState::kFailed;
		switch (a_layer) {
		case RuntimeState::kInitialized:
			return DisplayState::kActive;
		case RuntimeState::kFailed:
			if (a_failure == FailureKind::kDllMissing)
				return DisplayState::kDllMissing;
			if (a_failure == FailureKind::kVersionRejected)
				return DisplayState::kVersionRejected;
			return DisplayState::kFailed;
		case RuntimeState::kNotLoaded:
			break;
		}
		return DisplayState::kStarting;
	}
