...
void CODBloom::SetupResources()
{
	...
	logger::debug("Creating 2D textures...");
	{
		...
		for (uint i = 0; i < s_BloomMips; i++) {
			...
			Util::SetResourceName(texBloomMipRTVs[i].get(), "PostProcessing::CODBloom Mip %u RTV", i);
		}
	}
...
void CODBloom::ClearShaderCache()
{
	...
	BumpShaderGeneration();
	{
		...
		Util::ClearShaders<ID3D11PixelShader>({ thresholdPS, downsamplePS, downsampleFirstMipPS, upsamplePS, compositePS });
	}
...
void CODBloom::Draw(TextureInfo& inout_tex)
{
	...
	CS_GPU_PASS("PostProcessing::CODBloom");
	...
	// Set each destination before binding the source to avoid SRV/RTV aliasing.
...

	{
		...
		for (int i = s_BloomMips - 2; i >= 1; i--) {
			...
			if (i == static_cast<int>(s_BloomMips) - 2)
