	...
	struct VR_UpscaleDepthBuffer
	{
		static void thunk(RE::ImageSpaceManager* a_this, unsigned int a2, RE::RENDER_TARGET a_target, RE::RENDER_TARGET a_target2, __int64 a5, bool a6)
		{
			...
			if (renderer) {
				...
				if (uiDepth.texture) {
					globals::d3d::context->ClearDepthStencilView(uiDepth.views[0], D3D11_CLEAR_STENCIL, 0, 0);
					globals::d3d::context->ClearDepthStencilView(depth.views[0], D3D11_CLEAR_STENCIL, 0, 0);
				}
			}
		}
		static inline REL::Relocation<decltype(thunk)> func;
